              ...
              <div className='w-full md:w-1/2 space-y-8'>
                ...
                <div className="w-full grid grid-cols-2 gap-4">
                  {task?.assets?.map((el, index) => (
                    <img src={el} alt={task?.title} key={index} className="w-full rounded h-28 md:h-36 2xl:h-52 cursor-pointer transition-all duration-700 hover:scale-125" />
                  ))}
                </div>
              </div>
            </div>
  ...
  return (
    <div className='w-full flex gap-10 2xl:gap-20 min-h-screen px-10 py-8 bg-white shadow rounded-md justify-between overflow-y-auto'>
      ...
      <div className="w-full md:w-1/3">
        <h4 className="text-gray-600 font-semibold text-lg mb-5">Add Activities</h4>
        <div className="w-full flex flex-wrap gap-5">
          ...
          <textarea rows={10} value={text} onChange={(e) => setText(e.target.value)} placeholder="Type ......" className="bg-white w-full mt-10 border border-gray-300 outline-none p-4 rounded-md focus:ring-2 ring-blue-500"></textarea>
          {isLoading ? (
            <Loading />
          ) : (
